  ...
  for (const [node, depth] of dfs(ast)) {
    ...
    if (node.nodeType === 'text') {
      ...
      for (let i = start; i < end; i++) {
        ...
        const ratio = currentStyle.fontSize / sdfConfig.size;

        const vertexData = calculateVertices(x, y, width * ratio, height * ratio, sprite.transform.worldTransform, sprite._anchor, currentState.italic);
        
        x += (width - sdfConfig.buffer * 2) * ratio;
        y += 0 * ratio;
 ...
 */
function calculateVertices(x, y, width, height, worldTransform, anchor, italic = false) {
